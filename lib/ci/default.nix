...
let
  ...
  ciFiles = pkgs.lib.filter (x: !(pkgs.lib.hasSuffix "default.nix" x)) (file.getNixFiles ./.);
  allCI = (file.importLists' ciFiles) ++ [ CI ];

  # determine if a CI is detected
  isDetected = ci: pkgs.lib.any (env: builtins.getEnv env != "") ci.env;

  # return the first detected
  infoCI = pkgs.lib.findFirst isDetected { } allCI;
  inCI = infoCI != { };
in
{
  /**
