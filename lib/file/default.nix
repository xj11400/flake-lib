...
rec {
  ...
  */
  getDirs' =
    ...
    in
    pkgs.lib.attrNames (pkgs.lib.filterAttrs (n: v: v == "directory") entries);
  ...
  */
  getNixFiles' =
    ...
    let
      entries = builtins.readDir dir;
    in
    builtins.filter (name: pkgs.lib.hasSuffix ".nix" name) (builtins.attrNames entries);
    ...
    let
      ...
      safeMerge =
        ...
        let
          ...
          valueToMerge =
            if builtins.isAttrs content && !(pkgs.lib.isDerivation content) then
              content
            else
              { "${pkgs.lib.removeSuffix ".nix" (builtins.baseNameOf (toString file))}" = content; };
        in
        pkgs.lib.recursiveUpdate acc valueToMerge;
    in
    pkgs.lib.foldl' safeMerge { } allFiles;
  ...
  */
  importFiles =
    paths: args:
    let
      toKey = p: pkgs.lib.removeSuffix ".nix" (builtins.baseNameOf (toString p));
  ...
  */
  importAll' =
    ...
    let
      ...
      toKey = file: pkgs.lib.removeSuffix ".nix" (builtins.baseNameOf (toString file));

      safeMerge =
        acc: file:
        let
          content = import file args;
          isMergeable = builtins.isAttrs content && !(pkgs.lib.isDerivation content);
